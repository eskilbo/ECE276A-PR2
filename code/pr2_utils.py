...
def lidar_transforms():
  ...
  return RPY_Lidar,R_Lidar,T_Lidar

def fog_transforms():
  RPY_Fog = [0, 0, 0]
  R_Fog = [[1, 0, 0], [0, 1, 0], [0, 0, 1]]
  T_Fog = [-0.335, -0.035, 0.78]
  return RPY_Fog,R_Fog,T_Fog

def lidar():
  lidar_time, lidar_data = read_data_from_csv('data/sensor_data/lidar.csv')
  start_angle = -5
  end_angle = 185
  lidar_size = len(lidar_time)
  return lidar_size,lidar_time,lidar_data,start_angle,end_angle

def encoder():
  encoder_time, encoder_data = read_data_from_csv('data/sensor_data/encoder.csv')
  enc_left_count = encoder_data[:, 0]
  enc_right_count = encoder_data[:, 1]
  enc_resolution = 4096
  d_enc_left = 0.623479
  d_enc_right = 0.622806
  encoder_size = len(encoder_time)
  return encoder_size,encoder_time,encoder_data,enc_left_count,enc_right_count,enc_resolution,d_enc_left,d_enc_right
